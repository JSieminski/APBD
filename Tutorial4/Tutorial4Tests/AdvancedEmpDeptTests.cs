...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e.EName, e.Sal, e.DeptNo, DName = d.DName })
            .SelectMany(ed => grades.Where(g => ed.Sal >= g.Losal && ed.Sal <= g.Hisal).Select(g => new { ed.EName, ed.DName, Grade = g.Grade })).ToList();
